...
DECLARE
  ...
  v_lead int := 90; -- minimum minutes of lead time for same-day bookings
BEGIN
  ...
  -- Apply personal day-off / custom-hours exceptions.
  staff AS (
    SELECT bs.staff_id,
  ...
  -- Day workload per mechanic drives the round-robin rotation.
  load AS (
    SELECT st.staff_id,
  ...
  free AS (
    ...
    WHERE s.t >= v_min_time
      ...
      -- Workshop capacity: never exceed max parallel services at that moment.
      AND (
